  ...
  useEffect(() => {
    if (selectedQuery) {
      ...
      const channel = supabase
        .channel(`hod-query-messages-${selectedQuery.id}`)
        .on(
          'postgres_changes',
          {
            event: 'INSERT',
    ...
    <ProtectedRoute role="hod">
      <PageShell title="Student Queries" subtitle="Department query watchlist">
        <div className="grid gap-6 p-4 md:p-6 lg:grid-cols-[260px_minmax(0,1fr)] w-full min-w-0">
          <Sidebar active="/hod/queries" items={[{ href: '/hod', label: 'HOD Dashboard' }, { href: '/hod/faculty', label: 'Faculty & Mentors' }, { href: '/hod/students', label: 'Students' }, { href: '/hod/queries', label: 'Student Queries' }]} />
                  ...
                  <div className="flex-1 overflow-y-auto py-4 space-y-3 pr-1">
                    {loadingMessages ? (
                      ...
                      <div className="flex flex-col items-center justify-center h-full text-slate-400 text-center p-4">
                        ...
                        <p className="text-xs font-semibold">No messages yet. Send a message to guide the mentoring discussion.</p>
                      </div>
                    ) : (
                      messages.map((msg) => {
                        const isMe = msg.users?.role === 'hod';
                        const senderRole = msg.users?.role === 'student' ? 'Student' : msg.users?.role === 'faculty' ? 'Mentor' : 'HOD';
                        ...
                        return (
                          ...
                          >
                            <div className="text-[10px] text-slate-400 font-semibold mb-1 px-1">
                              {isMe ? 'You (HOD)' : `${msg.users?.name || 'User'} (${senderRole})`} • {new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </div>
                  ...
                  <div className="border-t border-slate-200 pt-3">
                    {selectedQuery.status === 'Resolved' ? (
                      <div className="flex items-center gap-1.5 rounded-2xl bg-slate-50 border border-slate-100 text-slate-600 p-3 text-xs font-semibold">
                        ...
                        <span>This query has been marked as Resolved.</span>
                      </div>
                    ) : (
