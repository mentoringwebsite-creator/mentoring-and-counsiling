...
export default function HodStudentsPage() {
  ...
  return (
    <ProtectedRoute role="hod">
      <PageShell title="Students" subtitle="Manage and monitor department students">
          ...
          <div className="space-y-6 w-full min-w-0">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex rounded-2xl bg-slate-100 p-1 border border-slate-200">
                ...
                <button onClick={() => setActiveTab('all')} className={`rounded-xl px-4 py-2 text-xs font-bold transition ${activeTab === 'all' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-600 hover:text-emerald-800'}`}>
                  All Approved
                </button>
              </div>
              ...
              <div className="relative w-full max-w-xs">
                ...
                <input type="text" placeholder="Search name or roll no..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full rounded-2xl border border-slate-300 bg-white py-2.5 pl-10 pr-4 text-sm focus:border-emerald-600 focus:outline-none" />
              </div>
            </div>
